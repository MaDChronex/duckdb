...
static unique_ptr<BaseStatistics> ListIdStats(ClientContext &context, FunctionStatisticsInput &input) {
...
ScalarFunction ListIdFun::GetFunction() {
	// the arguments and return types are actually set in the binder function
	// this is where the arguments are defined
	auto fun = ScalarFunction({LogicalType::LIST(LogicalType::ANY)},
	                          LogicalType::LIST(LogicalType::ANY), ListIdFunction, ListIdBind, nullptr,
	                          ListIdStats);
	fun.null_handling = FunctionNullHandling::SPECIAL_HANDLING;
	return fun;
}

void ListIdFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction({"list_id", "array_id"}, GetFunction());
}
